...
def water_effect(decode_array_to_img):
    waterEffect = cv2.stylization(decode_array_to_img, sigma_s = 60, sigma_r = 0.6)
    status, output_img = cv2.imencode('.PNG', waterEffect)
    return output_img
def HDR(decode_array_to_img):
    hdrEffect  = cv2.detailEnhance(decode_array_to_img, sigma_s = 12, sigma_r =0.15)
    status, output_img = cv2.imencode('.PNG', hdrEffect)
    return output_img
def invert(decode_array_to_img):
    invertEffevt = cv2.bitwise_not(decode_array_to_img)
    status, output_img = cv2.imencode('.PNG',invertEffevt )
    return output_img
